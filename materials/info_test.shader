...
uniform vec4 edge_tint : hint_color;
...
void fragment(){
	...
	float near_left = (UV.x + distort_sum.x) / (0.2 / sprite_scale.x);
	near_left = clamp(near_left, 0.0, 1.0);
	near_left = 1.0 - near_left;
	
	float near_right = (UV.x + distort_sum.x) * (3.0 * sprite_scale.x) - sprite_scale.x * 2.85;
	near_right = clamp(near_right, 0.0, 1.0);
	
	float near_bottom = (UV.y + distort_sum.y) * (3.0 * sprite_scale.y) - sprite_scale.y * 2.85;
	near_bottom = clamp(near_bottom, 0.0, 1.0);
	
	color = mix(color, edge_tint, near_top);
	color = mix(color, edge_tint, near_left);
	color = mix(color, edge_tint, near_right);
	...
	if(near_left > edge_lower){
		color.a = 0.0;
		
		if(near_left < edge_upper){
			color.a = (edge_upper - near_left) / (edge_upper - edge_lower);
		}
	}
	*/
